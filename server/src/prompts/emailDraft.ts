...
export function buildEmailDraftPrompt(params: {
  ...
  if (emailType === 'call_booked') {
    return `You are Jordan Bell from OxyScale (AI & Automation consultancy, based in Australia). You just had a call with ${leadName}${company ? ` at ${company}` : ''} and a meeting has been booked.
...
Write the email body only (no subject line, no "Hi ${firstName}," greeting as that is added automatically, no signature block). Keep it under 50 words.`;
  ...
  return `You are Jordan Bell from OxyScale (AI & Automation consultancy, based in Australia). You just called ${leadName}${company ? ` at ${company}` : ''}${category ? ` (industry: ${category})` : ''}.
...
- One line about who OxyScale is and what you do (AI and automation, tailored to their industry if you know it)
- One line about helping similar businesses in their space
- End with: "Would love to show you what we can do. Feel free to book a 30 min discovery call here: https://calendly.com/oxyscale/discovery"
...
- Match the ending to how the call actually ended:
  ...
  * If left open: one clear ask, or include the booking link: https://calendly.com/oxyscale/discovery
...
- Clear next step with the booking link: "Happy to run through it properly when you've got 30 minutes: https://calendly.com/oxyscale/discovery"
